...
const ChartServicePerformance: React.FC = () => {
  const series = [
    {
      name: "Vehicles Serviced",
      data: [168, 385, 201, 298, 187, 195, 291], // Example data: Number of vehicles serviced over the week
    },
  ];

  const options: ApexOptions = {
    colors: ["#57A1F3"], // Custom color to match a typical garage theme
    chart: {
    ...
    xaxis: {
      categories: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"], // Days of the week in Indian context
      axisBorder: {
        show: false,
      },
      axisTicks: {
    ...
    <div className="rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card">
      <div className="flex justify-between border-b border-stroke px-6 pb-4.5 pt-5.5 dark:border-dark-3">
        <div>
          ...
          <p className="text-body-sm font-medium">Service Performance of the Week</p>
        </div>
        <div>
          <h3 className="mb-0.5 text-body-2xlg font-bold text-dark dark:text-white">
            1,500 Vehicles
          </h3>
          <p className="flex items-center justify-end gap-1 text-right text-green">
            <svg
            ...
            <span className="text-body-sm font-medium">+2.0%</span>
          </p>
        </div>
      </div>

      <div className="px-6 pb-1 pt-7.5">
        <div id="chartServicePerformance" className="-ml-3.5">
          <ReactApexChart
            options={options}
            series={series}
            type="bar"
...
export default ChartServicePerformance;
